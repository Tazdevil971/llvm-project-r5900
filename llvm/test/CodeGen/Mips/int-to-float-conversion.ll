; RUN: llc -march=mipsel < %s | FileCheck %s -check-prefixes=ALL,32,32DF
; RUN: llc -march=mipsel -mattr=+single-float < %s | FileCheck %s -check-prefixes=ALL,32,32SF

; RUN: llc -march=mips64el -mcpu=mips4 < %s | FileCheck %s -check-prefixes=ALL,64,64DF
; RUN: llc -march=mips64el -mcpu=mips4 -mattr=+single-float < %s \
; RUN:   | FileCheck %s -check-prefixes=ALL,64,64SF

; RUN: llc -march=mips64el -mcpu=mips64 < %s | FileCheck %s -check-prefixes=ALL,64,64DF
; RUN: llc -march=mips64el -mcpu=mips64 -mattr=+single-float < %s \
; RUN:   | FileCheck %s -check-prefixes=ALL,64,64SF
